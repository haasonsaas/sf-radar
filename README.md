...
| `abc` | abc.ca.gov daily "Report of New Applications" and "Report of Issued Licenses" (scraped, not Socrata) | New liquor-license applications — a type 41/47 filing is a near-certain restaurant, earlier than any city record — and issued licenses, meaning opening is imminent |
...
| --- | --- |
| `sf-radar fetch [--full] [--since DATE]` | Incrementally pull all sources; `--full` backfills 90 days (14 report days for `abc`). A source that fails is skipped (the others still store) and the run exits non-zero listing the failures |
...
- **ABC liquor licenses**: applications score +3 for restaurant types (41 on-sale beer & wine eating place, 47 on-sale general eating place, 75 brewpub) and bar types (40/61 on-sale beer, 42/48 public premises), +2 for off-sale shop types (20/21); wholesale/importer/caterer types are dropped. An issued license scores one point higher than the application of the same type and is stored as a separate signal (`<license>-issued`), so a venue's timeline shows "application" and later "issued" — the lifecycle from filing to opening. The report's action code adjusts both: `ORI` (original license) and `PRM`/`PER/PRM` (the license is moving to this address) are new venues and keep full score; `PER` alone is a person-to-person transfer at the same premises — an ownership change, not an opening — and scores one lower (an off-sale transfer then drops below the storage threshold). This source is scraped from abc.ca.gov's daily statewide reports (two HTTP requests per report day, filtered to SF county, watermarked like the Socrata sources, 14-day initial backfill). A scrape failure prints a warning and skips the run instead of failing the whole fetch
...
**Corroboration**: at digest time, an entry gets +2 when other sources have filings at the same address (addresses are normalized across formats — case, whitespace, ST/STREET and DBI's two-letter permit abbreviations like BL/WY/HY, 1ST/FIRST, ONE/1 house numbers, 88-90/88 ranges, unit designators stripped), and +1 when other sources have filings under the same name (names normalized — case, punctuation, entity suffixes like LLC/INC dropped, plurals folded so BURGERS matches BURGER; minimum length 4). Name matches catch pairs whose addresses don't normalize equal, like a food truck's commissary vs its storefront. Corroboration matches against full DB history including already-seen rows, and the bonuses are display-only — stored scores are untouched. Only rows that are signals on their own (stored score ≥ 2) count as corroborators: business registrations and building permits are stored down to score 0, and an office tenant's DBA or a roof repair in the same tower is not evidence of a venue. Candidates are selected down to a stored-score floor of `--min-score` minus the maximum bonus (never below 1), so a permit stored at score 1 surfaces once another source files at its address; `business` rows are exempt from this rescue (a score-1 registration is any non-food DBA in the building). Rows that stay below `--min-score` after corroboration are not shown and not marked seen. Sources whose datasets carry no neighborhood (`abc`, `fire`, `planning`, `vending`) inherit one from any other filing at the same normalized address, and `--neighborhood` filters after that inheritance.

**Venues**: the digest displays venues, not filings. Unseen signals at the same normalized address cluster into one venue (a name match additionally attaches signals that have no usable address, such as a food-truck applicant, to the storefront — but two addresses never merge on name alone, so a chain's locations stay separate). A venue is named by its best DBA-bearing source (abc, health, business, … before synthetic "Permit N" names), scored by its best filing, and rendered as a timeline of its filings newest-first, followed by an `earlier:` line listing older filings at that address from full history. Every filing of a displayed venue is shown and marked seen, including ones below `--min-score` on their own.

Digest buckets: 🔥 venue score ≥ 4, 👀 score 2–3, · score 0–1 (the last only appears with `--min-score` below 2; JSON bucket `low`). Filings are labeled with their source, e.g. `[health]`. Permit-type and planning filings show a one-line snippet of the actual description (from the stored raw row, truncated to ~120 chars). Within a bucket, neighborhoods and venues are ordered by score then date, so the strongest signals come first. After printing, the digest also archives (marks seen) every unseen row older than the lookback window with a score at or above `--min-score` — the set the digest would have shown, shifted out of the window — and reports the count. Rows below `--min-score` stay unseen so a later digest with a lower threshold can still surface them; that digest marks them seen itself.
